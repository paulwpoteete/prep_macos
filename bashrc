...
#-------------------------------------------------------------------------------------------------------------------#
	...
	# PROMPT_COMMAND='echo -en "\e[2m\e[7m[Session Line:$LINENO Login:`date`]\n" '
...
then
	...
	### BSD PROMPT_COMMAND ###
	var_date=`date +%a,%Y%m%d.%H%Mhrs`
	var_ip=`/sbin/ifconfig -a | grep -v inet6 | grep inet| grep -v ":|127.0.0.1" | awk '{ printf $2", " }' | rev | cut -c 3- | rev`
	PROMPT_COMMAND='if [ ${EUID} == 0 ];
	then printf "[OS:`uname -s | sed s/Darwin/MacOS/g` Line:$LINENO Login:$var_date IP:$var_ip]\n";
	else printf "[OS:`uname -s | sed s/Darwin/MacOS/g` Line:$LINENO Login:$var_date IP:$var_ip]\n" ; fi'
fi
...
then
	...
	alias d='dict'
	alias yt='torify youtube-dl -if best'
	alias rsyncr='time rsync -huvr --remove-source-files --progress'
	alias ffmpeg='time ffmpeg -vsync 0 -hwaccel cuvid'
